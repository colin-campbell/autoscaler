...
func setupProvider(c config.Config) (autoscaler.Provider, error) {
	...
	case os.Getenv("OS_USERNAME") != "":
		return openstack.New(
			...
			openstack.WithAvailabilityZone(c.OpenStack.AvailabilityZone),
			openstack.WithNetworkType(c.OpenStack.NetworkType),
			openstack.WithNetworkId(c.OpenStack.NetworkId),
